...
const FOOBARDEBUG_FIELDS = [
  { key: 'test_text', label: 'Textfeld', type: 'string/text' },
  { key: 'test_number', label: 'Zahlenfeld', type: 'number' },
  { key: 'test_checkbox', label: 'Checkbox', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'foobar_debug', label: 'Foobar Debug', pascal: 'FoobarDebug' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('foobar_debug');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    foobar_debug: new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'foobar_debug': return (data as any).foobarDebug as FoobarDebug[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, _fieldKey: string, url?: unknown): string => {
    if (!url) return '—';
    const id = extractRecordId(url);
    if (!id) return '—';
    void getLookupLists(entity);
    return String(url);
  }, [getLookupLists]);

  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'foobar_debug': return FOOBARDEBUG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'foobar_debug': return {
        create: (fields: any) => LivingAppsService.createFoobarDebugEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFoobarDebugEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFoobarDebugEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
        ...
        {sel.size > 0 && (
          <div className="flex items-center gap-2 bg-muted/60 rounded-lg px-3 py-1.5">
            ...
            <Button variant="outline" size="sm" onClick={() => setBulkEditOpen(activeTab)}>
              <Pencil className="h-3.5 w-3.5 mr-1" /> Edit field
            </Button>
            <Button variant="outline" size="sm" onClick={() => handleBulkClone()}>
              <Copy className="h-3.5 w-3.5 mr-1" /> Clone selected
            </Button>
      ...
      {(createEntity === 'foobar_debug' || dialogState?.entity === 'foobar_debug') && (
        <FoobarDebugDialog
          open={createEntity === 'foobar_debug' || dialogState?.entity === 'foobar_debug'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'foobar_debug' ? handleUpdate : (fields: any) => handleCreate('foobar_debug', fields)}
          defaultValues={dialogState?.entity === 'foobar_debug' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['FoobarDebug']}
          enablePhotoLocation={AI_PHOTO_LOCATION['FoobarDebug']}
        />
      )}
      {viewState?.entity === 'foobar_debug' && (
        <FoobarDebugViewDialog
          open={viewState?.entity === 'foobar_debug'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'foobar_debug', record: r }); }}
        />
      )}
