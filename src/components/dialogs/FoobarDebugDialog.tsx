...
interface FoobarDebugDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: FoobarDebug['fields']) => Promise<void>;
  defaultValues?: FoobarDebug['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function FoobarDebugDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: FoobarDebugDialogProps) {
  const [fields, setFields] = useState<Partial<FoobarDebug['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'foobar_debug');
      await onSubmit(clean as FoobarDebug['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "test_text": string | null, // Textfeld\n  "test_number": number | null, // Zahlenfeld\n  "test_checkbox": boolean | null, // Checkbox\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<FoobarDebug['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Edit Foobar Debug' : 'New Foobar Debug';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="test_text">Textfeld</Label>
            <Input
              id="test_text"
              value={fields.test_text ?? ''}
              onChange={e => setFields(f => ({ ...f, test_text: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="test_number">Zahlenfeld</Label>
            <Input
              id="test_number"
              type="number"
              value={fields.test_number ?? ''}
              onChange={e => setFields(f => ({ ...f, test_number: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="test_checkbox">Checkbox</Label>
            <div className="flex items-center gap-2 pt-1">
              <Checkbox
                id="test_checkbox"
                checked={!!fields.test_checkbox}
                onCheckedChange={(v) => setFields(f => ({ ...f, test_checkbox: !!v }))}
              />
              <Label htmlFor="test_checkbox" className="font-normal">Checkbox</Label>
            </div>
          </div>
          <DialogFooter>
